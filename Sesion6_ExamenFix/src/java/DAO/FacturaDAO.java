...
public void borrarFactura (Connection con,Factura factura) throws Exception
{     PreparedStatement stmt=null;                         
    try {
        //Creamos la orden MySQL para el borrado del registro
        stmt= con.prepareStatement("DELETE FROM `articulo_factura` WHERE `Factura_idFactura` = ?");
        stmt.setInt(1, factura.getId());
        stmt.executeUpdate();
        ...
        ex.printStackTrace();
        throw new Exception("Ha habido un problema al BORRAR la factura "+ex.getMessage());
    } finally
    {
        if (stmt != null) stmt.close();
    } 
