...
public class FacturaBLL {
    ...
    public void borrarFactura2(int idFactura) throws Exception{
        
            Connection _con=null;
            Conexion_DB _conexion_DB = new Conexion_DB();
            _con = _conexion_DB.AbrirConexion();
            FacturaDAO factura_DAO = new FacturaDAO();
            factura_DAO.borrarFactura2(_con, idFactura);
            _conexion_DB.CerrarConexion(_con);
            
    }
    public void borrarLineaArticulo(Cliente cliente, Factura factura, Articulo articulo, int cantidad) throws Exception
        
    {
        ...
        ClienteDAO _clienteDAO=new ClienteDAO();
        cliente=_clienteDAO.findByDNI(_con, cliente);//Recuperamos los datos del cliente
        float _precioCompra=cantidad*articulo.getPrecio();
        FacturaDAO facturaDAO=new FacturaDAO();
        facturaDAO.borrarLineaArticulo(_con, factura, articulo);//Inserta la linea de factura
    ...
    public ArrayList<Factura> getFacturas(Cliente cliente) throws Exception
    {
            Connection _con=null;
            Conexion_DB _conexion_DB = new Conexion_DB();
            _con = _conexion_DB.AbrirConexion();// Abrimos la conexión
            FacturaDAO facturaDAO= new FacturaDAO(); 
            ArrayList <Factura> facturas = facturaDAO.getFacturas(_con, cliente);
