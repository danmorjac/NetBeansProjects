    ...
    <table height="100%"border=2 width="100%">
      <tr>
        <td  height="5%" bgcolor=blue colspan=2>
          <table width=100%>
            <tr>
              <td style="color:white" align="center" width="75%">
                Aplicación web
              </td>
              <td style="color:white" align="right">
                <%
                  ...
                  if (request.getParameter("sesion") != null) {
                    // Si esta iniciada LA BORRA, por que he llegado aqui desde
                    // el boton "log out"
                    request.getSession().removeAttribute("ClienteSesion");
                  } else {
                    // Si no esta iniciada pues imprime el nombre del logeado
                    Cliente _clienteSesion
                      = (Cliente) request.getSession()
                      .getAttribute("ClienteSesion");
                    if (_clienteSesion != null) {
                %>
                <%=_clienteSesion.getNombreCompuesto()%>
              </td>     
              <td>
      ...
      <tr>
        ...
        <td width="75%">
          <%String _advertencia = (String) request.getAttribute("advertencia");
            // Si hay advertencia la muestra
            if (_advertencia != null) {
          %>
          <label style="color:red"><%=_advertencia%></label>
          <%}

            String paginaprincipal = 
              (String) request.getAttribute("paginaprincipal");
            // Si hay componente a mostrar, lo muestra
            if (paginaprincipal != null) {%> 
          <jsp:include page='<%=paginaprincipal%>' />
          <%}%>                
        </td>
      </tr>
      <tr>
